...
// Updated dummy customer data
const initialCustomers = [
  { id: 1, name: "Acme Corp", cohort: "Jan 2026", monthlyLTV: 12000, yearlyLTV: 9500, monthlyRisk: "High Risk", yearlyRisk: "Safe", monthlyProb: 85, yearlyProb: 15, factors: ["Logins dropped by 45%", "2 recent payment failures"], emailGenerated: false },
  { id: 2, name: "NovaTech Solutions", cohort: "Feb 2026", monthlyLTV: 8500, yearlyLTV: 7800, monthlyRisk: "Safe", yearlyRisk: "Safe", monthlyProb: 12, yearlyProb: 8, factors: ["Consistent daily usage", "Upgraded plan recently"], emailGenerated: false },
  { id: 3, name: "SaaSify Ltd", cohort: "Jan 2026", monthlyLTV: 15000, yearlyLTV: 11000, monthlyRisk: "High Risk", yearlyRisk: "Medium Risk", monthlyProb: 78, yearlyProb: 45, factors: ["Support ticket volume increased 3x", "Key features unused"], emailGenerated: false },
  { id: 4, name: "Quantum Labs", cohort: "March 2026", monthlyLTV: 5400, yearlyLTV: 5200, monthlyRisk: "Safe", yearlyRisk: "Safe", monthlyProb: 5, yearlyProb: 5, factors: ["High team collaboration", "Zero support tickets"], emailGenerated: false },
];
    ...
    <main className="p-8 relative">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-8 relative">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50/70 border-b border-slate-100 text-xs font-semibold text-slate-500 uppercase tracking-wider">
                  <th className="p-4 pl-6 cursor-pointer hover:text-slate-900" onClick={() => handleSort('name')}>Client Name {sortConfig.key === 'name' && (sortConfig.direction === 'asc' ? '↑' : '↓')}</th>
                  <th className="p-4">Cohort</th>
                  <th className="p-4 cursor-pointer hover:text-slate-900" onClick={() => handleSort('ltv')}>Calculated LTV {sortConfig.key === 'ltv' && (sortConfig.direction === 'asc' ? '↑' : '↓')}</th>
              ...
              <tbody className="divide-y divide-slate-50 text-sm text-slate-700">
                {processedCustomers.map((customer) => {
                  ...
                  return (
                    <tr key={customer.id} onClick={() => setSelectedCustomer(customer)} className="hover:bg-slate-50 transition-colors cursor-pointer">
                      ...
                      <td className="p-4">
                        <div className="flex items-center gap-3">
                          <span className={`text-xs font-semibold px-2.5 py-1 rounded-full w-24 text-center ${currentRisk === 'High Risk' ? 'bg-rose-100 text-rose-700' : currentRisk === 'Medium Risk' ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'}`}>{currentRisk}</span>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col min-h-[400px]">
          ...
          {generating ? (
            <div className="flex-1 flex flex-col items-center justify-center text-slate-400"><span className="w-8 h-8 border-2 border-indigo-600/30 border-t-indigo-600 rounded-full animate-spin mb-3"></span><p className="text-xs font-medium">Drafting predictive template...</p></div>
          ) : activeEmail ? (
            <div className="flex-1 flex flex-col"><div className="text-xs text-slate-400 mb-2 font-medium">Targeting: {activeEmail.clientName}</div><textarea className="flex-1 w-full p-4 bg-slate-50 border border-slate-200 rounded-xl text-xs font-mono text-slate-700 leading-relaxed focus:outline-none focus:border-indigo-500 resize-none" value={activeEmail.body} readOnly /><button className="w-full mt-4 py-2.5 bg-indigo-600 text-white font-semibold text-sm rounded-xl hover:bg-indigo-700 transition-colors shadow-sm">Copy Email Template</button></div>
          ) : (
            <div className="flex-1 border-2 border-dashed border-slate-200 rounded-xl flex flex-col items-center justify-center p-6 text-center text-slate-400"><svg className="w-8 h-8 mb-3 opacity-60" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z"></path></svg><p className="text-xs font-medium">Click on any high risk client's "AI Email" button to compile a tailored retention plan.</p></div>
          )}
        </div>
      </div>
        ...
        <>
          ...
          <div className="fixed inset-y-0 right-0 w-full md:w-96 bg-white shadow-2xl z-50 transform transition-transform duration-300 flex flex-col border-l border-slate-200">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <div><h2 className="text-xl font-bold text-slate-900">{selectedCustomer.name}</h2><p className="text-xs text-slate-500 font-medium mt-1">Cohort: {selectedCustomer.cohort}</p></div>
              <button onClick={() => setSelectedCustomer(null)} className="p-2 text-slate-400 hover:bg-slate-200 rounded-full transition-colors"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg></button>
            </div>
            <div className="p-6 flex-1 overflow-y-auto">
              <div className="mb-8">
                <h3 className="text-xs font-semibold uppercase tracking-wider text-slate-400 mb-4">Financial Vectors</h3>
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-100"><p className="text-xs text-slate-500 mb-1">Monthly LTV</p><p className="text-lg font-bold text-slate-900">${selectedCustomer.monthlyLTV.toLocaleString()}</p></div>
                  <div className="bg-slate-50 p-4 rounded-xl border border-slate-100"><p className="text-xs text-slate-500 mb-1">Yearly Adjusted LTV</p><p className="text-lg font-bold text-slate-900">${selectedCustomer.yearlyLTV.toLocaleString()}</p></div>
                </div>
              </div>
              <div className="mb-8">
                ...
                <div className="space-y-3">
                  {selectedCustomer.factors.map((factor, idx) => (
                    <div key={idx} className="flex items-start gap-3 p-3 bg-rose-50/50 rounded-xl border border-rose-100/50"><svg className="w-5 h-5 text-rose-500 mt-0.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg><p className="text-sm text-slate-700">{factor}</p></div>
                  ))}
                </div>
              </div>
            </div>
            <div className="p-6 border-t border-slate-100 bg-slate-50">
               <button onClick={(e) => { generateEmail(e, selectedCustomer.id, selectedCustomer.name); setSelectedCustomer(null); }} className="w-full py-3 bg-indigo-600 text-white font-semibold rounded-xl hover:bg-indigo-700 transition-all shadow-sm">Draft Retention Email</button>
            </div>
          </div>
        </>
      )}
